...
if [ $? -eq 0 ]; then
   ...
   kubectl create secret generic kafka-access --from-literal=apikey=$KAFKA_API_KEY --from-literal=topic=$KAFKA_TOPIC_NAME --from-literal=broker1host=$KAFKA_BROKER1_DNS --from-literal=broker1port=$KAFKA_BROKER1_PORT -n ${STOCKTRADER_NAMESPACE}
   if [ $? -eq 0 ]; then
      echo "Kafka proxy setup completed successfully"
   else
